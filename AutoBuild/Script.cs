        ...
        void Moved(InvDT Inv, ref Dictionary<InvDT, List<MyInvItem>> tp, List<MyInvItem> Lims = null)
        {
            ...
            for (int j = inv.Count - 1; j >= 0; j--)
            {
                ...
                while (stor.GetInv_Move(Pi.Lnk, Inv.Inv, ref i) >= 0 && p != null)
                {
                    ...
                    /*if (res) p = null;
                    else */if ((p = Inv.Inv.GetItemByID(inv[j].ItemId)) != null) Pi.count -= (double)p.Amount;
        ...
        public void SetPanel(string param, IMyTextPanel txt = null)
        {
            ...
            else
            {
                ...
                { var pn = sl.Change(x).FindBlock<IMyTextPanel>(GridTerminalSystem);
                  if (pn!=null) pn.ShowPublicTextOnScreen(); return pn; }, p2[1], out tmp);
