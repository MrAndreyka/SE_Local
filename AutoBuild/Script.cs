/*----------------------------------------------------------------------    
    ...
    void Main(string arg, UpdateType UT)
    {
        ...
        if (TM.Run() == 0 && UT >= UpdateType.Update1) return;
    ...
    void Moved(InvDT Inv, ref Dictionary<InvDT, List<MyInvItem>> tp, List<MyInvItem> Lims = null)
    {
        ...
        IMyInventoryItem p;
        List<IMyInventoryItem> invs = Inv.Inv.GetItems();
        if (inv.Count == 0) return;

        bool res = false;
        List<MyInvItem> TecL;
        for (int j = invs.Count - 1; j >= 0; j--)
        {
            if ((p = invs[j]).Amount == 0) continue;
            ...
            while (p != null && stor.GetInv_Move(Pi.Lnk, Inv.Inv, ref i) >= 0)
            {
                ...
                if ((p = Inv.Inv.GetItemByID(p.ItemId)) != null) Pi.count -= (double)p.Amount;
        ...
        try
        {
            ...
            switch (arg.ToLower())
            {
                ...
                case "panel+":
                    {
                        var p = param.Split(':');
                        if (p.Length != 2) { Echo("Ожидается 2 параметра: <Блок приемник>:<Маска поиска добавляемых блоков>"); return; }
                        ...
                        var L1 = GridTerminalSystem.GetBlockGroupWithName(p[0]) as IMyTextPanel;
                        if (L1 == null) { Echo($"Панель \"{p[0]}\" не найдена"); return; }

                        FindPlane tc; TextPanel tp;
                        if (!Out.FindPanel(x => x.CustomName == param, out tc, out tp)) { Echo($"Панель \"{param}\" еще не установлена"); return; }
                        ...
                        for (var i = 0; i < L.Count; i++) { TecPan.Add(new TextPanel(L[i])); Echo("+ " + L[i].CustomName); L[i].ShowPublicTextOnScreen(); }
                    }
                    break;
                case "reload":
                    {
                ...
                case "unload":
                    {
                        ...
                        new Selection(param).FindBlocks(GridTerminalSystem, l, x => x.HasInventory && !InvData.IsSpecBloc(x));
                ...
                case "download":
                    {
                        ...
                        param = args.Length > 0 ? args[0] : null;
                        List<IMyTerminalBlock> l = new List<IMyTerminalBlock>();
                        new Selection(param).FindBlocks(GridTerminalSystem, l, x => x.HasInventory && !InvData.IsSpecBloc(x));
                        if (l.Count == 0) { Echo($"Не найдены блоки по запросу: {param}"); return; }
                        ...
                        if (args.Length > 1)
                        {
                            lm = new List<MyInvItem>();
                            for (var i = 1; i < args.Length - 1; i += 2)
                            {
                                var tr = LangDic.Find(x => x.StartsWith(args[i], true, null));
                                if (string.IsNullOrEmpty(tr.Key)) { Echo($"Не найдено значение отбора: {args[i]}"); continue; }
                                if (!tr.Value.StartsWith("*"))
                                    lm.Add(new MyInvItem(new MyInvIt(tr.Key, true, 0), float.Parse(args[i + 1])));
                                else
                                    lm.Add(new MyInvItem(new MyInvIt(tr.Key, true, MyInvIt.GetType(tr.Key)), float.Parse(args[i + 1])));
                            }

                            List<MyInvItem> ListCount = new List<MyInvItem>();
                            CalAndMove(ListCount, false);

                            ListCount.ForEach(Pi =>
                            {
                                var tmp = lm.FindLast(z => z.Lnk.IsSuitable(Pi.Lnk));
                                if (tmp == null || (tmp.Lnk.Type == 0 && tmp.count <= 0)) return;
                                if (tmp.Lnk.Type != 0) lm.Add(tmp = new MyInvItem(new MyInvIt(Pi.Lnk.Name, true), tmp.count - Pi.count));
                                else tmp.count -= Pi.count;
                            });
                        }
                        else lm = null;

                        var Dil = new Dictionary<InvDT, List<MyInvItem>>();
                        l.ForEach(x => Moved(new InvDT(x, (byte)(x.InventoryCount - 1)), ref Dil, lm));
                        Echo(">>>>>>>>" + Dil.Count);
                        ShowMoved(Dil);
                        if (Out.Uncown.Length > 0) { Echo(Out.Uncown.ToString()); Out.Uncown.Clear(); }
                    }
    ...
    {
        var p2 = param.Split(':');
        if (p2.Length != 2 && txt == null) { Echo("Ожидается 2 параметра: <отбор>:<шаблон панели>"); return; }
        ...
        {
            int tp = Abs_Plane.TryParse(x =>
            {
                var pn = sl.Change(x).FindBlock<IMyTextPanel>(GridTerminalSystem);
                if (pn != null) pn.ShowPublicTextOnScreen(); return pn;
            }, p2[1], out tmp);
    ...
    void Restore(string value, int i = 0)
    {
        ...
        while (i < cou && !string.IsNullOrWhiteSpace(val[i]))
        {
            ...
            Out.Add(new FindPlane(FindList.Parse(st[0]), Abs_Plane.Parse(x => GridTerminalSystem.GetBlockWithId(long.Parse(x)) as IMyTextPanel, st[1])));
        }
    ...
    public class Sklads : List<MyRef>
    {
        ...
        public int GetInv_Move(FinderItem It, IMyInventory Inv, ref int i)
        {
            ...
            if (beg == 0)
            {
                ...
                if (i >= 0 && this[i].IsInclude(It)) return i = -1;
            }
            for (i = beg; i < Count; i++)
            {
