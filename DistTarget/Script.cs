...
static Camers сamers = new Camers();
Timer timer;
long TimeSkan = 0;
...
Program()
{
	...
	if (ars.Length > 0)
	{
		Txt = new Selection(ars[0]).FindBlock<IMyTextPanel>(GridTerminalSystem);
		if (Txt == null) Echo($"Не найдена панель \"{ars[0]}\"");
	}

	new Selection(null).FindBlocks<IMyCameraBlock>(GridTerminalSystem, сamers);
	сamers.ForEach(x => x.EnableRaycast = true);

	timer = new Timer(this);
	Echo("Инициализировано\nКамер найдено: " + сamers.Count);
}
...
void Main(string arg, UpdateType UT)
{
	...
	try
	{
		...
		{ // ручной запуск
			...
			if (Target.Type != MyDetectedEntityType.SmallGrid && Target.Type != MyDetectedEntityType.LargeGrid) { Mess($"Цель на растоянии {Dist} м. не найдена"); return; }

			Mess(string.Format("{0} ({1}) {2}\n{3}", Target.Name, Target.Type, Target.Position, MyGPS.GPS("Цель", Target.Position)), true);
			...
			if (ant.TransmitMessage("Kill:" + Target.Position))
			{
				Mess("Отправка сообщения антеной -" + ant.CustomName, true);
				TimeSkan = 0;
				timer.SetInterval(960, true);
			}
			else Mess("Отправка сообщения не удалась", true);
			return;
		}
		 ...
		 // Отслежавание цели
		{
			...
			var cam = сamers.GetCamera(TarPos);
			if (cam != null)
			{
			...
			if (Target.IsEmpty()) { timer.Stop(); Mess("Цель потеряна"); return; }
			else if (cam!=null) Mess(cam.CustomName + "\n" + (cam.GetPosition() - Target.Position).ToString(),false,false);
			else Mess("Камеры не готовы..." + сamers.tec +"_"+ TimeSkan + MyGPS.GPS("Ц"+ Target.TimeStamp, TarPos));
		}
	}
	catch (Exception e)
	{
...
public void Mess(string text, bool consol = false, bool added = true)
{
